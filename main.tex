...
The development of an effective scheme to mitigate the adverse impacts induced by the bus bunching problem relies on understanding the bus movement and depicting bus trajectories. Existing methods inadequately address two issues, namely, capacity constraint and passengers travel behavior in multi-line scenarios.
...
Numerical examples are conducted to examine the effects of passenger transfer behavior and bus capacity constraint on the propagation of bus bunching. It is found that the propagation of bus bunching problem from a disturbed bus line to an undisturbed bus line via the common corridor is enhanced because of the presence of transfer passengers. This implies that \textcolor{red}{motivating passengers }. Meanwhile, using smaller bus capacity vehicles may lead more regular service as bus dwell times are constrained to avoid severe bus bunching.
